...
$(function () { //获取处理友链数据
    $.getJSON("../json_data/daohang.json", function (data) {

        $('.links-content').html("");        
        let notValid = data.filter((item, a, b) => item.valid == 0);
        let notValid1 = data.filter((item, a, b) => item.valid == 1);     
    if (notValid.length > 0) {
        $('.links-content').append("<h4 class=\"text-gray\"><i class=\"linecons-tag\" style=\"margin-right:7px\" id=\"办公常用\"></i>办公常用</h4>");
     var html="";
    ...
    if (notValid1.length > 0) {
        $('.links-content').append("<h4 class=\"text-gray\"><i class=\"linecons-tag\" style=\"margin-right:7px\" id=\"速查手册\"></i>速查手册</h4>");
     var html="";
